...
| **Nabe drehsymmetrisch per Konstruktion: `zeiger/nabe-v9.webp`** (seit v0.83) | Lutz am 21.9.: „Scheibe zwei auf Scheibe drei, das ist die ganze Zeit das Problem", dazu „das Loch ist nicht richtig rund". Nach v5 lagen die Ringe noch 0,26…2,26 px neben der Mitte — aber **die Richtungen zeigen kreuz und quer** (99°, 190°, 263°, 272°, 104°, 40°, 294°…), der Median über alle Kanten ergibt nur −0,29 px. Zwei Läufe desselben Kantensuchers auf derselben Datei streuen um 0,7 px. **Damit ist die Messgrenze erreicht — weiter messen bringt nichts.** Deshalb der Kategoriewechsel: `v9` wird aus dem entzerrten `v5` als **eindimensionales Radialprofil** gebildet (Median über 1440 Winkel) und einmal herumgedreht. Danach *kann* keine Scheibe mehr neben einer anderen sitzen und kein Loch mehr unrund sein. Der Median verwischt die Ringe um genau so viel, wie sie versetzt waren; das wird am 1-D-Profil per unscharfer Maske (k=2,2 bei σ=2,2) wieder herausgerechnet, bis die Kantenenergie der Vorlage entspricht (**1340 gegen 1351**). Die Schärfung läuft längs des Radius und kann die Symmetrie nicht brechen. ⚠️ Der Preis ist ein flacherer, „abgedrehter" Eindruck — im Labor steht `v5` unter **„Nabe: Textur"** weiter zur Wahl. |
| **Gescheiterte Zwischenschritte am Nabenbild** (nicht wiederholen) | `v4`: Median direkt aus der schiefen Vorlage — exakt symmetrisch, aber die Ringe verwischt. `v7`: dichte Registrierung je Radius — machte es **messbar schlechter** (mittlerer Versatz 2,07 statt 0,98 px), weil der starke Außenring die Fensterkorrelation dominiert. `v8`: Profile je Winkel vor dem Mitteln ausrichten — nicht schärfer als v6, weil die Verzerrung keine Verschiebung je Winkel ist, sondern Ovalität, die mit dem Radius wächst. |
| **Nabe rund: `zeiger/nabe-v3.webp`** (seit v0.80, abgelöst) | Lutz am 20.9.: „oder die Naben sind nicht exakt rund??" — ja. Die Vorlage ist **4,0 % höher als breit** (waagerecht 158,9, senkrecht 165,2; Radiusstreuung 6,90 px). `nabe-v2` hatte das nur mit einer Kreismaske weggeschnitten und blieb bei 1,8 %. `v3` **streckt das Bild vorher in der Breite** um 165,2/158,9 = 1,0396 und maskiert erst danach. Gemessen: Radius 160,2…161,8, Streuung **0,33 px**, oval **+0,01 %**, Loch 0,4 px von der Mitte. ⚠️ Beim nächsten Messingbild als Erstes die Rundheit prüfen — die Vorlagen sind gerendert, nicht konstruiert. |
